...
# Try to import required modules, and install them if not found
try:
    from tkinter import *
    ...
    print(f"{missing_package} is missing. Installing...")
...
def getvals():
    ...
    c = var.get()
    if c == 0:
        tmsg.showwarning("Warning", "Click on the checkbox before submit")
    elif c == 1:
        file_path = "form_details.xlsx"
        if not os.path.exists(file_path):
            wb = Workbook()
            ws = wb.active
            headers = ["Name", "Father's Name", "Date Of Birth", "Department", "Gender", "Enter PH No", "Email id", "Aadhar No", "Permanent Address", "Current Address", "10th Marks", "12th Marks"]
            ws.append(headers)
            wb.save(file_path)
        ...
        ws.append(new_row)
        wb.save(file_path)
        tmsg.showinfo("Submit", "You have successfully submitted the form")
